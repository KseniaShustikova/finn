/* global themeBgColors, themeColor, jQuery, wp, _ */
/**
...
( function() {
	...
	wp.customize.bind( 'ready', function() {
		...
		wp.customize( 'accent_hue', function( value ) {
			value.bind( function( to ) {
				...
				Object.keys( themeBgColors ).forEach( function( context ) {
					var backgroundColorValue;
					if ( themeBgColors[ context ].color ) {
						backgroundColorValue = themeBgColors[ context ].color;
					} else {
						backgroundColorValue = wp.customize( themeBgColors[ context ].setting ).get();
					}
					themeSetAccessibleColorsValue( context, backgroundColorValue, to );
				} );
			} );
		} );
		...
		Object.keys( themeBgColors ).forEach( function( context ) {
			wp.customize( themeBgColors[ context ].setting, function( value ) {
				value.bind( function( to ) {
					// Update the value for our accessible colors for this area.
					themeSetAccessibleColorsValue( context, to, wp.customize( 'accent_hue' ).get(), to );
				} );
			} );
		} );

		// Show or hide retina_logo setting on the first load.
		themeSetRetineLogoVisibility( !! wp.customize( 'custom_logo' )() );

		// Add a listener for custom_logo changes.
		wp.customize( 'custom_logo', function( value ) {
			value.bind( function( to ) {
				// Show or hide retina_logo setting on changing custom_logo.
				themeSetRetineLogoVisibility( !! to );
			} );
		} );
	} );
	 ...
	 * @return {void}
	 */
	function themeSetAccessibleColorsValue( context, backgroundColor, accentHue ) {
		var value, colors;
		...
		colors = themeColor( backgroundColor, accentHue );
	 ...
	 * @return {void}
	 */
	function themeSetRetineLogoVisibility( visible ) {
